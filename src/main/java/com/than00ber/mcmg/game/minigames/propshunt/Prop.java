...
public class Prop {
    ...
    public Prop(Player player) {
        this(player, player.getLocation(), null, 0, HiderStatus.UNDECIDED);
    }

    public Prop(Player player, Location playerLocation, Material material, int ticks, HiderStatus status) {
        this.player = player;
    ...
    public Prop setHidingSpot(Location playerLocation) {
        this.playerLocation = playerLocation;
        return this;
    }

    public Prop setHidingMaterial(Material material) {
    ...
    public Prop increaseTimer(int hidingTickThreshold) {
    ...
    public static void revealToAll(Player prop, Location location) {
        showToOthers(prop.getPlayer());
        location.getBlock().setType(Material.AIR);
    }

    public static Block disguiseFromAll(Player prop, Material material) {
        hideFromOthers(prop.getPlayer());
        Block blockToHide = prop.getLocation().getBlock();
        blockToHide.setType(material);
        return blockToHide;
    }

    public static void showToOthers(Player prop) {
        for (Player player : Main.INSTANCE.getServer().getOnlinePlayers()) {
            player.hidePlayer(Main.INSTANCE, prop);
        }
    }

    public static void hideFromOthers(Player prop) {
        for (Player player : Main.INSTANCE.getServer().getOnlinePlayers()) {
            player.hidePlayer(Main.INSTANCE, prop);
        }
    }
